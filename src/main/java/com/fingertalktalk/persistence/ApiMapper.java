...
public interface ApiMapper {
    //회원가입
    @Insert("INSERT INTO member(member_id, member_pw, member_name, member_phone, member_dep)" +
            "VALUES (#{member_id}, #{member_pw}, #{member_name}, #{member_phone}, #{member_dep})")
    boolean addMemberInfo(MemberVO member);

    //ID로 회원정보 가져오기
    /*@Param 안의 "member_id" 는 Controller 에서 Mapper 를 호출할때 넣어주는 파라미터와 이름이 동일해야함*/
    @Select("SELECT member_id, member_pw, member_name, member_phone, member_dep from member where member_id = #{member_id}")
    ...
    @Update("UPDATE member SET member_pw = #{member_pw}, member_name = #{member_name}," +
            " member_phone = #{member_phone}," + "member_dep = #{member_dep} where member_id = #{member_id}")
