...
public interface MallMapper {
    ...
    @Insert("INSERT into mall(mall_name, server_name, mall_img, auto_login_url, solution) values(#{mall_name}, #{server_name}, #{mall_img}, #{auto_login_url}, #{is_cafe24})")
    boolean addMallInfo(MallVO mallVO);

    @Update("UPDATE mall SET mall_name = #{mall_name}, server_name = #{server_name}, mall_img = #{mall_img}, auto_login_url = #{auto_login_url}, solution = #{solution} WHERE mall_idx = #{mall_idx}")
